...
update_mock_abis:
	...
	@curl -s -X POST -H "Content-Type: application/json" -d '{"account_name":"eosio.msig"}' https://eos.greymass.com/v1/chain/get_abi | (command -v jq >/dev/null && jq '.abi // {}' || python -c "import sys, json; data = json.load(sys.stdin); print(json.dumps(data.get('abi', {}), indent=4))") > test/data/abis/eosio.msig.json
