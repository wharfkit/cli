...
import {addKeyToWallet, generateDefaultKeyName, listWalletKeys} from './utils'
...
export function listKeys(): void {
    try {
        ...
        if (keys.length === 0) {
            log('No keys found in wallet.', 'info')
            log('Create a new key with: wharfkit wallet keys create', 'info')
            return
        }

        log(`Found ${keys.length} key(s) in wallet:\n`, 'info')

        keys.forEach((key, index) => {
            log(`${index + 1}. ${key.name}`, 'info')
            log(`   Public Key: ${key.publicKey}`, 'info')
            log(`   Created: ${new Date(key.createdAt).toLocaleString()}`, 'info')
            if (index < keys.length - 1) {
                log('', 'info')
            }
        })
    } catch (error) {
        log(`❌ Failed to list keys: ${(error as Error).message}`, 'info')
        process.exit(1)
    }
}

/**
 * Create a new key in the wallet
 */
export async function createKey(options: KeysCreateOptions): Promise<void> {
    try {
