...
suite('generateContractFromCommand', () => {
    ...
    test('--json option uses the provided ABI', async function() {
      try {
        ...
        assert.fail(`Error should not be thrown when valid JSON ABI file is passed. Error: ${error}`)
      }
    });
    ...
    test("--json option throws an error when file doesn't exist", async function() {
      try {
        ...
        assert((error as unknown as Error).message.includes('no such file or directory'), 'Error should be thrown when JSON file does not exist.');
      }
    })
    ...
    test('--file option specifies the path for the generated file', async function() {
      ...
      assert(writeFileSyncStub.calledWith('./path/to/file', sinon.match.string), 'writeFileSync should be called with correct path');
    });
    ...
    test('Without --file option, code is logged to stdout.write', async function() {
      ...
      assert(stdOutStub.calledWith(sinon.match.string), 'process.stdout.write should be called with generated contract code');
    });
});
